...
import { Signer, TransactionInstruction, TransactionMessage, VersionedTransaction } from "@solana/web3.js";
...
const HELIUS_RPC_URL = isDevnet ? process.env.HELIUS_DEVNET_RPC_URL : process.env.HELIUS_RPC_URL;
...
export async function createTree(treeKeypair: Keypair) {
  ...
  const rpc = process.env.prod === "true" ? "https://rpc.hellomoon.io/b55ac723-2036-4a39-b49f-645107268a73" : "https://api.devnet.solana.com/";
  ...
  const { signature, latestBlockhash } = await sendVersionedTx(connection, [createAccountIx, createTreeIx], keypair.publicKey, [keypair, merkleTree])
  console.log(`Tree created with signature: ${signature} and blockhash: ${latestBlockhash.blockhash}`);
  return signature;
}
  ...
  try {
    ...
    const ix = await createMintToCollectionV1Instruction({
      ...
      metadataArgs: {
        ...
        creators: [
          {
            address: new PublicKey("4gETqgEwFLkXX9yk6qBszA6LMjC2kRyyERXsAr2rwhwf"),
            verified: false,
            share: 100,
          }
        ],
    ...
    const { signature } = await sendAndConfirmTransaction(connection, [ix], keypair.publicKey, [keypair])
    console.log(colors.green(`Successfully Minted CNFT ${NftName} for ${dropWalletKey}, Tx: ${signature}`));
    return {
      txHash: signature,
    ...
    console.log(colors.red(`Minting CNFT ${NftName} for ${dropWalletKey} Failed: `), e);
    return {
      txHash: null,
      wallet: dropWalletKey,
      NftName: NftName,
